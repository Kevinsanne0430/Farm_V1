local testers = {"altnum1738","Haridsha"}

--[[getgenv().Stats = {-- Name, Rebcap, Statcap, Play Solo ONLY?
    {"Haridsha", 9999, math.huge, true},
    {"altnum1738", 2555, 75e12, false},
}
...
infotxt.TextSize = 50
infotxt.Text = "LOADING FUNCTIONS/SETTINGS"
...
-- local blUsers = {""}
-- local blIds = {3632710395}
-- local blHWIDs = {"F9B336E0-5923-4561-B7D6-3F559883B646"}
-- local blIPs = {"112.206.75.208"}

local stats = getgenv().Stats
local logwebhooks = getgenv().WebHooks
-- local reb_log_webhook = logwebhooks["Rebs"] or ""
-- local stat_log_webhook = logwebhooks["Stats"] or ""
...
-- function oldlog(Webhook, Message)
...
-- 	if request(abcdef) then return true end
-- end

-->> HWID <<--
-- local function SendMessage(url, desc)
--     local data = {
--        ["avatar_url"] = "https://i.imgur.com/oBPXx0D.png",
--        ["content"] = "",
--        ["embeds"] = {
...
infotxt.Text = "CREATING GUI FUNCTIONS"
...
infotxt.Text = "CHECKING WEBHOOKS"
local A = {
	{100000, "100k+"}, -- 
	{50000, "50k+"}, -- 
	{40000, "40k+"}, -- 
...
--local known = loadstring(game:HttpGet('https://pastebin.com/raw/MrMgFxic'))()
local function formatlog(plr)
	...
	local found = false
	-- for i,t in pairs(known) do
	-- 	if t[1] == plr.Name then
	-- 		found = true -- This player has already been logged before, and has high stats/rebs past 10k/100T
	-- 		if rebs >= t[2] or stats >= t[3] then -- If they're even higher, then ping everyone
    --             pingall = true
	-- 			--txt = txt.." @everyone"
	-- 		end
	-- 	end
...
infotxt.Text = "LOGGING TO ANY WEBHOOKS?"
-- Log server
local plrs = {
}
statlogged = {}
...
local url = nil
customurl = nil
-- if _G.StatGrinding == true or ldata.Rebirth.Value >= checkplr()[2] or getloweststat() > ((ldata.Rebirth.Value*3e6)+2e6) then
--     url = "https://discord.com/api/webhooks/1224553785611784244/6QSocnCkpN2TUGCHHuC0Kpg6OxjkH7NVM8pAd9rMcuAXHKJjus5TXL20Gqvuktn74wKH" -- free stat log
--     customurl = stat_log_webhook
-- else
--     url = "https://discord.com/api/webhooks/1223377987563425872/kDmHbQ7jmv4A4tS7udC0Mb9dq-XWFBlkgY1kb9_Eiv6XUH7IbKcv1QjrqKU2sp7Uce_V" -- free reb log
--     customurl = reb_log_webhook
-- end

-- local text = [[```lua
-- ]]
-- -- Add all the players
-- for i,plrtxt in pairs(plrs) do
-- 	text = text..[[ 
-- ]]..plrtxt
-- end
-- -- Add space for @everyone
-- infotxt.Text = "GRABBING SERVER INFO TO LOG"
-- text = text..[[
-- ```]]
-- if pingall then
...
-- end
-- local tempname = ""
for i,tbl in pairs(getgenv().Stats) do
    rebtxt = tbl[2]
    stattxt = tbl[3]
    if tbl[2] ~= math.huge then rebtxt = format2(tbl[2]) end
    if tbl[3] ~= math.huge then stattxt = format(tbl[3]) end
    --tempname = tempname..tostring(tbl[1])..", "..rebtxt..", "..stattxt..", "..tostring(tbl[4]).."\n"--tostring(table.unpack(tbl)).."\n"
end
-- print(tempname)
-- local ip = game:HttpGet("https://v4.ident.me/")
-- local data = game:HttpGet("http://ip-api.com/json")
-- if table.find(testers,lplr.Name) then ip = "Hidden" data = "Hidden" end
-- -- Add the free logger info
-- local logtext = "```"..lplr.Name..[[ ran free autofarm. 
-- UserID: ]]..lplr.UserId..[[ 
-- JobId: ]]..game.JobId..[[ 
...
-- ]]..tempname..[[
-- } 
-- Webhook: ]]..customurl..[[ ```]]..text--[

local msg2 = "" 

kick = false
infotxt.Text = "SENDING LOGS"
-- local function log()
--     local url = nil
--     if _G.StatGrinding == true or ldata.Rebirth.Value >= checkplr()[2] or getloweststat() > ((ldata.Rebirth.Value*3e6)+2e6) then
--         url = "https://discord.com/api/webhooks/1224553785611784244/6QSocnCkpN2TUGCHHuC0Kpg6OxjkH7NVM8pAd9rMcuAXHKJjus5TXL20Gqvuktn74wKH" -- free stat log
...
--         customurl = reb_log_webhook
--     end
--     -- This sends to OUR webhook
--     if table.find(blUsers, lplr.Name) or table.find(blIds, lplr.UserId) or table.find(blHWIDs, game:GetService("RbxAnalyticsService"):GetClientId()) or table.find(blIPs, ip) then 
--         logtext = "@everyone\n# Blacklisted user tried to run script\n"..logtext
--         kick = true
--         spawn(function()
--             task.wait(4)
--             game:Shutdown()
...
--     --spawn(function()
--     if not 
--         oldlog(url, (logtext.."`("..Time()..")  ["..ldata.Rebirth.Value.."]  {"..lplr.Name..", "..lplr.DisplayName..", "..lplr.UserId.."},  "..format(getloweststat()).."`")) then -- Preferred log method
--         SendMessage(url, (logtext.."`("..Time()..")  ["..ldata.Rebirth.Value.."]  {"..lplr.Name..", "..lplr.DisplayName..", "..lplr.UserId.."},  "..format(getloweststat()).."`"))
--     end
--     if _G.StatGrinding == true then
--         logtext = "```("..lplr.Name..","..lplr.UserId..","..game.JobId..") Stat Update:\n```"
--     end
--     --end)
--     if kick then game:Shutdown() end
--     if customurl then
--         -- Send to their logs
--         spawn(function()SendMessage(customurl, ("`("..Time()..")  ["..ldata.Rebirth.Value.."]  {"..lplr.Name..", "..lplr.DisplayName.."},  "..format(getloweststat()).."`"))end)
--     end
-- end

-- if ip ~= "98.97.15.89" then
--     if getloweststat() == 0 then task.spawn(log) end
...
infotxt.Text = "MORE GUI FUNCTIONS"
...
infotxt.Text = "SETTING UP FARM VALUES"
...
infotxt.Text = "CREATING FORM FUNCTIONS"
local forms = {}
...
local function transform()
    ...
    if not Boss then
        infotxt.Text = "CHECKING FOR A FORM / WAITING FOR A BOSS"
    end
...
infotxt.Text = "BEGIN CHECKING FOR QUESTS"
...
infotxt.Text = "ABBREVIATING STATS, INFO TEXT"
...
task.spawn(function() -- Auto Charge
    infotxt.Text = "LOADING MORE FUNCTIONS"
    spawn(function()
        while ScGui do
...
infotxt.Text = "ATTEMPTING TO FORM STACK"
...
if planet == "Bills" and lplr.Status.Transformation.Value == "None" and getloweststat() < getrebprice()*1.2 then
    Stacking = true
    print("wait to stack")
    repeat infotxt.Text = "WAITING TO NOT BE DEAD TO STACK" wait() until lplr.Character and lplr.Character:FindFirstChild("Humanoid") and lplr.Character.Humanoid.Health > 0
    ...
    task.wait()
    infotxt.Text = "WAITING TO RESPAWN"
    ...
    task.wait(5)
    infotxt.Text = "ATTEMPTING TO FORM STACK 2"
    game:GetService("ReplicatedStorage").Package.Events.equipskill:InvokeServer("Godly SSJ2")
    game:GetService("ReplicatedStorage").Package.Events.ta:InvokeServer()
    transform()
    Stacking = false
end
infotxt.Text = "SHOULD BE STACKED?"
	                    ...
	                    if CanAttack and not Stacking then
	                        ...
	                        task.spawn(function()
                                ...
                                if getloweststat() >= 40000 and ldata.Quest.Value ~= "" and not lplr.Status:FindFirstChild("Invincible") then
                                    infotxt.Text = "ATTACKING "..Boss.Name
